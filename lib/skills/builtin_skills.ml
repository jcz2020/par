...
let builtin_skills : skill_descriptor list = [
  {
    ...
    system_prompt_override = Some "You are a meticulous code reviewer. Analyze code for correctness, security, performance, and style. Provide specific, actionable feedback with file:line references.";
  ...
  };
  {
    ...
    system_prompt_override = Some "You are an expert summarizer. Distill information into clear, concise summaries. Preserve key facts, drop redundancy. Use bullet points for readability.";
  ...
  };
  {
    ...
    system_prompt_override = Some "You are a professional translator. Translate accurately while preserving tone and context. If the target language is ambiguous, ask for clarification.";
    tool_filter = All_tools;
  ...
  };
  {
    ...
    system_prompt_override = Some "You are a RAG assistant. Answer questions using the provided retrieved context. If the context doesn't contain the answer, say so clearly. Always cite which document the information came from.";
