...
let split_frontmatter (content : string) : string option * string =
  ...
  if String.length normalized > prefix_len && String.sub normalized 0 prefix_len = prefix
  then begin
    let rec find_close i =
      if i + prefix_len > String.length normalized then None
      else if String.sub normalized i prefix_len = prefix then Some i
      else find_close (i + 1)
    in
    match find_close prefix_len with
    | None -> (None, normalized)
    | Some close_idx ->
      let fm_raw = String.sub normalized prefix_len (close_idx - prefix_len) in
      let body_start = close_idx + prefix_len in
      let body =
        if body_start < String.length normalized
        then String.sub normalized body_start (String.length normalized - body_start)
        else ""
      in
      (Some fm_raw, body)
  end
