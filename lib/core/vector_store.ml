  ...
  | Result.Ok () ->
    (match init_schema db dimension with
     | Result.Error e ->
       ...
       Result.Error e
     | Result.Ok () ->
       Result.Ok { db; mutex = Eio.Mutex.create (); dimension })
...
let add t docs =
  if docs = [] then Result.Ok ()
    ...
    Eio.Mutex.use_rw t.mutex ~protect:true (fun () ->
      match exec_sql t.db "BEGIN TRANSACTION" with
      | Result.Error e -> Result.Error e
      | Result.Ok () ->
        let errors = ref [] in
        List.iter
          (fun ({ id; content; metadata }, vec) ->
            ...
            if rc <> Sqlite3.Rc.DONE then begin
              errors :=
                sqlite_error_to_category
                  ("doc insert: " ^ Sqlite3.errmsg t.db)
                :: !errors
            end;
            let rowid =
              Sqlite3.last_insert_rowid t.db
            in
            let vec_json =
            ...
            if rc2 <> Sqlite3.Rc.DONE then begin
              errors :=
                sqlite_error_to_category
                  ("vec insert: " ^ Sqlite3.errmsg t.db)
                :: !errors
            end)
          docs;
        match !errors with
        | [] ->
          let _ = exec_sql t.db "COMMIT" in
          Result.Ok ()
        | e :: _ ->
          let _ = exec_sql t.db "ROLLBACK" in
          Result.Error e)

let search t ~query ~k =
  if Array.length query <> t.dimension then
    invalid_arg
      (Printf.sprintf
