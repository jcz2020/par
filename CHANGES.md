...
- **PAR-qyr** (P0, **UNVERIFIED**): CJK input workaround — removed linenoise (which processed UTF-8 bytes individually) and reverted to `input_line stdin` where terminal handles UTF-8 natively in canonical mode. Added `strip_ansi_escapes` to filter CSI + SS3 escape sequences from arrow/Home/End/Delete keys. **WARNING: This has NOT been manually tested in a real TTY.** The fix relies on terminal native UTF-8 handling, which should work on modern terminals with `IUTF8` flag set, but the original PAR-r05 bug was reported against a similar setup. PAR-qyr remains OPEN until a human verifies CJK input and deletion in a real terminal. See `docs/v0.4.6-ROADMAP.md` for the mandatory QA gate.
