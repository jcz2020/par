# P-A-R (Programmable Agent Runtime) 完整设计 v1.1
...
module type PERSISTENCE_SERVICE = sig
  type t
  val save_events : t -> event list -> (unit, error_category) result Eio.Fiber.t
  val load_events : t -> Task_id.t -> (event list, error_category) result Eio.Fiber.t
  val save_task_state : t -> task_state -> (unit, error_category) result Eio.Fiber.t
  val load_task_state : t -> Task_id.t -> (task_state option, error_category) result Eio.Fiber.t
  val transaction : t -> (t -> 'a Eio.Fiber.t) -> ('a, error_category) result Eio.Fiber.t
end
...
**修复说明**：原设计 `runtime_context = { db_conn : Caqti_eio.connection option; ... }` 导致工具直接操作 DB、测试困难、实现锁定。新设计通过模块类型约束，工具仅获得所需服务接口。
...
```ocaml
val transition :
  (module PERSISTENCE_SERVICE with type t = 'p) ->
  task_state -> task_status ->
  (task_state, string) result Eio.Fiber.t
...
val htbl_get : protected_hashtbl -> 'k -> 'v option Eio.Fiber.t
val htbl_set : protected_hashtbl -> 'k -> 'v -> unit Eio.Fiber.t
val htbl_remove : protected_hashtbl -> 'k -> unit Eio.Fiber.t
val htbl_iter : protected_hashtbl -> ('k -> 'v -> unit) -> unit Eio.Fiber.t
```
...
val graceful_shutdown : runtime -> shutdown_config -> int Eio.Fiber.t
```
...
val flush_events :
  ...
  event_envelope list ->
  (unit, error_category) result Eio.Fiber.t
```
...
val apply_context_strategy :
  ...
  conversation ->
  (conversation, error_category) result Eio.Fiber.t
```
...
val create : config:runtime_config -> (runtime, error_category) result Eio.Fiber.t
val close : runtime -> int Eio.Fiber.t
```
...
val submit_task : runtime -> task_input -> ?priority:int -> ?timeout:float ->
  Task_id.t Eio.Fiber.t

val wait_for_task : runtime -> Task_id.t -> timeout:float ->
  (Yojson.Safe.t option, [> `Timeout | `Task_failed of error_category ]) result Eio.Fiber.t

val get_task_status : runtime -> Task_id.t -> (task_status option, error_category) result Eio.Fiber.t
val cancel_task : runtime -> Task_id.t -> (unit, error_category) result Eio.Fiber.t
val approve_task : runtime -> Task_id.t -> approver:string -> (unit, error_category) result Eio.Fiber.t
...
val submit_workflow : runtime -> workflow ->
  (Workflow_run_id.t, error_category) result Eio.Fiber.t
val get_workflow_status : runtime -> Workflow_run_id.t ->
  (workflow_status, error_category) result Eio.Fiber.t
val cancel_workflow : runtime -> Workflow_run_id.t -> (unit, error_category) result Eio.Fiber.t
```
