...
| 2026-07-17 | **v0.7.5: 纯 OCaml HNSW 向量库**（架构正确，非范围妥协） | 用户质疑"OCaml 不能做了吗"——重新评估后确认纯 OCaml HNSW 比外部服务（Qdrant/Milvus）更符合 PAR "self-contained runtime" 定位。**架构正确（R1）**：`Vector_store.t` 内部类型改为 variant（`Sqlite_vec_backend \| Hnsw_backend`），runtime-selectable via `vector_store_backend` ADT，跟 `llm_service`/`memory_service` 模式一致。零外部依赖（不需要部署 Qdrant/Milvus），跨平台（Windows 不需要 vec0.dll）。**影响范围**：`lib/core/hnsw.ml`（新）、`types.ml`（新 ADT）、`vector_store.ml`（variant dispatch）、`runtime.ml`（`?vector_store_backend` 参数）。**回退方式**：sqlite-vec 仍是默认后端；HNSW 是 opt-in。Python FFI 暴露推迟到后续版本（OCaml SDK only）。 |
| 2026-07-17 | **v0.7.5: .docx 纯 OCaml 加载器** | 新增第 6 种文档格式（.txt/.md/.html/.csv/.pdf → .docx）。用户选纯 OCaml（camlzip + xmlm）而非 pandoc shell-out，保持 self-contained。**依赖级变更**：新增 camlzip + xmlm 硬依赖。**影响范围**：`lib/documents/docx_loader.ml`（新）、`directory_loader.ml`（`.docx` 加入 `default_map`）。 |
| 2026-07-17 | **v0.7.5: OpenAI/Anthropic 原生结构化输出** | `complete_structured_fn` 从 `None` 改为 `Some`，启用 OpenAI `response_format: json_schema`（strict mode）和 Anthropic `output_config`。Ollama/Custom 保持文本注入 fallback。**API/接口级变更**：FFI 层 `par_capi.ml` 行为变更（provider 原生模式替代 fallback），但签名不变。 |
| 2026-07-17 | **v0.7.5: CI 改进** | Python 绑定测试加入 CI 矩阵（Python 3.10-3.13）；Windows CI 条件化 `--with-test`（避免 Cygwin 测试依赖链失败）；`lib/ffi/dune` 移除 `eio.unix`（POSIX 专用，未被 FFI 代码使用）。 |
...
| 2026-07-19 | **【文档滞后清单】v0.7.5–v0.7.8 STRATEGY.md §8 摘要（retroactive）** | v0.7.5 (HNSW + .docx + native structured output) / v0.7.6 (Python parity + FFI GC fix + YAML block scalars) / v0.7.7 (downstream root-cause fixes + FFI save/update_current exposure) / v0.7.8 (PyPI wheel pipeline restored) 在 CHANGES.md 有详细 section,但 STRATEGY.md §8 变更日志未及时同步——本条目作为 retrospective marker,详细条目由后续 `docs: sweep STRATEGY.md §8 for v0.7.5-v0.7.8` commit 补齐（follow-up,非阻塞）。当前 §8 重点条目是 v0.7.9 egress wrap（架构级,需 R1-R5 论证）;v0.7.5-v0.7.8 多为 dependency/CI/FFI 层改动,R1-R5 论证在 CHANGES.md 中已隐含。 |
