...
| 2026-08-07 | v0.8.6 — 流式 HTTP 错误透传修复 | Bug：流式 LLM 调用收到非 2xx HTTP 响应时静默吞掉错误（`do_request_streaming` 缺状态码检查，错误体被当作 SSE 解析后丢弃，返回 `Ok { chunks_received = 0 }`）。修复：在 HTTP 客户端层补状态码检查并 raise `Http_status_error`，激活两个 provider 已存在但属于死代码的异常处理器。1504 tests。**决策**：用户选择直接发稳定版 v0.8.6（跳过 beta），暂不推 opam-repository。Pre-Bump Gate 用 `FORCE=1`（v0.8.6 不在 ROADMAP）。 |
| 2026-08-15 | **【API级+架构级变更】一等取消 + 可行动工具错误（v0.10.0 stable, 2026-08-15 发布）** | **变更前**: 取消通过 `Eio.Cancel.Cancelled` 异常传播，错误面混杂 `Timeout` / `Internal`，无法区分用户主动取消与超时；工具拒绝消息只说"rejected"不解释原因，模型在 ReAct 循环中无法学会正确重试；`edit` 工具 `old_text` 未找到时静默成功（最严重缺陷类别）。**变更后**: (1) 新增 `cancel_reason` ADT（`User_cancelled | Guard_cancelled of string`）+ `error_category.Cancelled of cancel_reason` 变体；`Cancellation.request_cancel` 改为 `cancel_reason -> unit`（先到先得）；取消在三个检查点生效（循环顶部/工具分派前/逐 chunk）；取消返回 `Error (Cancelled reason, recovered_conversation)` 且对话始终 provider 重放有效。(2) 新增 `tool_error.ml` 模块：`[workspace]`/`[bash-policy]`/`[cancelled]` 标记，四种 workspace 拒绝消息（路径遍历/冒号/绝对路径在外/受保护位置），`edit` 工具 `old_text` 未找到报错，bash 策略拒绝指出策略名和命令名。设计原则：在拒绝点教学而非 system prompt。(3) Python dispatch-queue trio（`invoke_start`/`invoke_poll`/`invoke_cancel`）。**影响范围**: `types.mli`（新 ADT + 新 error_category 变体）、`cancellation.mli`（`request_cancel` 签名变更）、`engine.ml`（三处检查点 + 合成消息）、`tool_error.ml`（新模块）、`builtin_tools.ml`（所有文件工具 + bash + edit 描述更新）、`bindings/python/runtime.py`（三个新方法）、6 个 doc 文件（EN+ZH）。**回退方式**: revert 相关 commits，恢复 `request_cancel` 为 unit-param，删除 `tool_error.ml`，恢复 `edit` 静默成功行为。 |

---

## 9. 相关文档

- `docs/v0.3-ROADMAP.md`（内部文档） — 当前路线图
...
| 2026-07-11 | **【依赖级+范围级变更】v0.7.3 stable: 审计清零（38 问题）+ §11 R2 遗留清零** | **变更前**: v0.7.0–v0.7.2 期间累积 48 审计问题未清（含 10 P0 真 bug、6 P1 竞态/死代码、10 P2 文档签名错误、12 P3 质量债）。`run_agent_structured` 需求悬而未决。**变更后**: 38 问题全部修复（10 P0 + 6 P1 + 10 P2 + 12 P3 全部清零；2 个审计时未识别的小问题 #5+ #32+ #c0 内化到 review 后续修复中）,6 个原子提交（按模块组 A-F）push 到 main。`docs/audit/v0.7.x-findings.md` 标记"全部已修复于 v0.7.3"。CHANGES.md 加 v0.7.3 section。**§11 R1 标注(38 条)**: 全 P0/P1 为「架构正确」——Auto 模式智能解析、update 真删旧行、invoke_structured 补 Invoke_context、FFI 锁外分配、cancel CAS、double-appendix 元数据追踪——均为原设计意图的执行层修复,无架构重新决策。**§11 R1 标注(12 条 P2 文档)**: 全「架构正确」——SDK 文档与 .mli 签名对齐零摩擦,中英双语同步,`mkdocs build --strict` 通过。**§11 R2 范围妥协(2 条)**: (D3 评审遗留) `LOADER` 模块类型删除（用户确认选 a,选项 b 「真实现」需要 R3(a) 破坏 SemVer major,选项 c 「技术未知」R5 不适用）→ 长远方案: 真实现 `Seq.t` 流式加载需有真实消费端驱动,目前无 RAG 流水线按 Doc 逐个消费——R2 合规,无退役日期,等真实需求出现再启动；（D4 评审遗留）加密 PDF 检测基于 regex 搜 "encrypt|password" 异常消息——功能可用但脆弱（camlpdf 改错误消息文本即失效）→ 长远方案: 捕获 `Pdf.PDFError` 异常类型并匹配 "Encryption" 前缀——v0.7.4 已部分落实（用类型匹配替 regex）。**§11 R3 两步走合规性**: 38 问题均在 0.7.x 范围内修复（无 SemVer major 破坏,minor 在 0.x 允许含 breaking 符合 §4）。首个 stable tag v0.7.3。1387 tests passing,Oracle 验证通过。 |

| 2026-07-11 | **【API级+架构级+依赖级变更】v0.7.4 stable: json_extract 顺序修复 + run_agent_structured（两阶段 ReAct + 结构化）** | **变更前**: (a) `extract_json_from_text` 处理顺序 bug——`strip_think_tags` 在位置 0 留下空白,`strip_markdown_fences` 检查 `s.[0:3]=="\`\`\`"` 因前导 `\n` 而失败（fallback `find_balanced_block` 碰巧兜底,但脆弱）；(b) `run_structured` 把 tools 传给 LLM 但不执行——LLM 返回 tool_calls 被静默忽略,repair loop 失败后报 Invalid_input——agents 需要"工具+结构化"无路径。**变更后**: (a) 重排处理链为 `strip_think_tags → trim → strip_markdown_fences → trim`（去掉首个冗余 trim）;加 2 个测试覆盖 think+fence/reasoning+fence;(b) 新增 `Engine.run_agent_structured`——两阶段模式: Phase 1 `run_agent` 跑完整 ReAct 循环（带工具）→ Phase 2 `run_structured` 拿完整对话历史做结构化调用（不带工具）;`Runtime.invoke_structured` 和 `workflow_engine.Agent_call` 路由:`config.tools <> []` 走 `run_agent_structured`,`config.tools = []` 走 `run_structured`（轻量路径不变）。**§11 R1 标注**: (a) json_extract 处理链重排为「架构正确」——trim 的语义位置（"消除前后空白"）与 think-tag 剥离的副作用（位置 0 留白）天然冲突,trim 必须在 strip 之后才正确;(b) 两阶段为「架构正确」——LangGraph `create_react_agent(response_format=)` 文档原话:"The graph will make a separate call to the LLM to generate the structured response after the agent loop is finished"——与 PAR 设计对齐;`run_structured` 保留作为轻量路径（纯结构化输出无需 ReAct 循环开销）——非妥协,是不同 use case。**§11 R1 标注(关键决策)**: 选 LangGraph 两阶段（业界验证,CrewAI v1.9.0→v1.9.1 教训:交错式在 vLLM/非 OpenAI provider 上工具被跳过）而非 OpenAI Agents SDK 交错式（tools + response_format 同发,仅 OpenAI 原生 API 可靠）。**§11 R2 范围妥协**: 无——两阶段是终态设计,R3(a) 不破坏 SemVer(major 破坏仅在 0.x 范围外适用),R3(c) 无未知技术验证,R3(d) 用户明确指示选两阶段。**§11 R5 适用**: 无未知技术——LangGraph/CrewAI/OpenAI Agents SDK 三个生产级框架已有公开模式对比。1387 tests,新增 3 个测试覆盖 run_agent_structured 的三种场景（有工具+schema、无工具 delegation、Phase 1 error propagation）。Engine.mli 导出新函数。 |

| 2026-07-19 | **【API级+架构级变更】v0.7.9 stable: Engine.run_agent terminal Assistant 消息物化 — egress wrap 根因修复** | **变更前**: `Engine.run_agent` 有 7 个 Ok 终态分支,6 个 inline 调 `add_assistant_message` 后 return,1 个（`_` catch-all for `finish_reason = Stop | Content_filter`）**漏调**。结果: `Runtime.invoke` 返回的 `invoke_result` 中 `response.text` 持有最终 assistant 内容,但 `conversation.messages` 缺少对应 `{role = Assistant; ...}` entry。下游影响: conversation resume / audit trail / `run_agent_structured` Phase 2 context 静默不完整。**变更后**: 三层架构重构—— (1) **Loop invariant**（docstring at `engine.ml:587-596`）: conv 在 loop 内 MUST NOT 持有 terminal response; (2) **删除 4 个 inline terminal appends**（lines 617, 973, 980, 988）+ 保留 mid-iteration appends（Tool_calls at L790, Continue sub-chunks at L~1022 — feeds next LLM call's context, different concern）; (3) **单一 egress wrap** at `run_agent` 出口（`engine.ml:1137-1149`）: 唯一一次 `add_assistant_message` 调用,带 idempotency check（仅在 last message 已是 terminal-shaped Assistant 且 text 匹配 resp.text 时 skip,覆盖 Branch 6 Continue compact 路径）。**Branch 6 (Max_tokens/Continue) normalize**: 中间 chunks（initial Max_tokens resp + user_feedbacks + cont_resp_*)被 compact 成单个 combined_msg whose text = final_resp.text,顺带修复一个 latent bug（pre-fix `final_resp.text` combined vs `final_conv.messages` 末尾 last chunk 不一致）。**Handoff + empty-text collision hole closed**（third-round Oracle audit）: idempotency check 原本仅匹配 `role = Assistant`,在 `Handoff carry_context=true` 时 source agent 的 mid-iter tool_calls Assistant（content_blocks=[], text_of_message=""）会 survive in carried context,若 target agent 返回空 Stop（text=None,restrictive prompt 下真实模型行为）,naive role-only check false-match 在 "" = "" 上,skip target's terminal append。收紧为 `tool_calls = None AND tool_call_id = None` 才能进 skip path,structurally 排除 mid-iter tool_call Assistant。**§11 R1 标注**: 「架构正确」——loop invariant + 单一 egress 是终态设计,消除"remember-to-call" anti-pattern（已产生一个 missing-branch bug,未来还会产生更多）。Oracle 三轮 audit 一致 VERIFIED。**§11 R1 最强反驳**（Oracle 自我 rebut）: "fix replaces one human-memory burden (remember to call) with another (remember NOT to call inside loop)"——但原 bug class 是 under-append（silent missing data,最坏）,新 bug class 是 over-append（visible duplicate,任何 smoke test 抓）。Asymmetry 倾向 fix,net improvement。**§11 R2 范围妥协**: OCaml 没法真正 type-enforce（phantom type 才能完全 close）,docstring + 删除 default + idempotency check 三层近似。Future maintainer 在 loop 内 inline append 不同 text 会产生 silent duplicate——code review 是唯一 guard。Acceptable: 新加 terminal branch 的 default 路径是 `Ok (resp, conv)`,自动正确——这是 structural win。**§11 R5 不适用**: bugfix（最小化层面）+ 架构决策（egress wrap 是 root-cause 路径）混合,但 R5 专为 ROADMAP scope,bugfix 走最小化原则已合规。**Error-path 行为变化**: pre-fix 不一致（仅 Max_tokens/Retry 路径有 partial append）,post-fix 统一为"no Error path 有 terminal append"——更一致。Blast radius: 无 test/caller 依赖 pre-fix 行为,无 documented dependency。**影响范围**: `lib/core/engine.ml`（loop invariant docstring + 4 处 append 删除 + Branch 6 compact + egress wrap + idempotency check）,`test/test_engine_assistant_message.ml`（11 tests covering Stop/Content_filter/Tool_calls→Stop/Max_tokens variants/Handoff/Generate/Resume）,`test/manual_qa_assistant.ml`（dune-built diagnostic executable）,`CHANGES.md` v0.7.9 section。1418 OCaml tests passing（新增 11）。Oracle三轮 VERIFIED（empirically reproduced hole + green-with-fix）。**回退方式**: revert commits 9a1bf26 + 0e835a1 + 10416b8 + 771ed61。**审计教训**: 第一轮 + 第二轮 audit 都漏了 handoff + empty-text collision hole,第三轮 skeptical verification 才发现——多轮 audit 是必要的,单轮 review 即使 expert 也会漏 edge case。 |

| 2026-07-17 | **【API级+架构级+依赖级变更】v0.7.5 stable: HNSW + .docx + 原生结构化输出** | **变更前**: (a) RAG 向量存储仅 sqlite-vec 后端（需 vec0.so/.dylib/.dll 外部二进制,Windows 需 vendor DLL）; (b) document loaders 缺 .docx 格式支持（用户需自行提取 Word 文档文本）; (c) 结构化输出走 text-injection fallback（LLM 在 system prompt 里被指示输出 JSON,而非用 provider 原生 json_schema 能力）。**变更后**: (a) **纯 OCaml HNSW 向量库**（`lib/core/hnsw.ml`,Algorithms 1-4 from Malkov & Yashunin TPAMI 2020）——零外部二进制依赖,跨平台（Windows 不需 vec0.dll）,cosine + L2 distance,configurable M/ef_construction/ef_search,Marshal 二进制持久化;`Types.vector_store_backend` ADT（`Vs_sqlite_vec | Vs_hnsw`）runtime-selectable,跟 `llm_service`/`memory_service` 模式一致;`Runtime.create ?vector_store_backend` + `Vector_store.create_for_backend` 工厂分发; (b) **.docx loader**（`lib/documents/docx_loader.ml`）——camlzip 解 ZIP + xmlm 流式解析 `word/document.xml`,提取 `<w:t>` 文本,段落→换行,tab 保留,field 指令排除;新增 deps camlzip + xmlm; (c) **原生结构化输出**——`complete_structured_fn` 从 `None` 改为 `Some`,OpenAI 用 `response_format: json_schema`（strict mode）,Anthropic 用 `output_config`,Ollama/Custom 保持 fallback。**§11 R1 标注**: (a) HNSW 为「架构正确」——用户质疑"OCaml 不能做了吗"后重新评估,纯 OCaml HNSW 比外部服务（Qdrant/Milvus）更符合 PAR "self-contained runtime" 定位;vector_store_backend variant 与 llm_service/memory_service 模式对齐; (b) .docx 选纯 OCaml（camlzip+xmlm）而非 pandoc shell-out,保持 self-contained; (c) 原生模式替代 fallback 是 provider 能力的正确利用。**§11 R2 范围妥协**: vector_store_backend/HNSW/Docx_loader 在此版本仅 OCaml SDK 可用,Python FFI 暴露推迟到 v0.7.6——R2 合规（FFI 暴露是独立工作,不阻塞 OCaml SDK 用户）,退役触发：v0.7.6 周期。**影响范围**: `lib/core/hnsw.ml`+`hnsw.mli`（新）+ `lib/core/vector_store.ml`（variant dispatch）+ `lib/core/types.ml`（新 ADT）+ `lib/core/runtime.ml`（`?vector_store_backend`）+ `lib/documents/docx_loader.ml`+`mli`（新）+ `lib/ffi/par_capi.ml`（`complete_structured_fn = Some`）+ 10 个新测试。1408 tests。**回退方式**: revert commit ff6d399。 |

| 2026-07-18 | **【API级+依赖级变更】v0.7.6 stable: Python parity + FFI GC root registration + YAML block scalars** | **变更前**: v0.7.5 的 HNSW/.docx/原生结构化输出仅 OCaml SDK 可用,Python binding 无法使用;`par_ffi.c` 多个 C bridge 函数缺 CAMLparam/CAMLlocal GC root 注册（OCaml 5.x GC 可能在 C callback 期间移动 OCaml 堆对象）;Python `_normalize_config` 缺 v0.7.5 新字段（event_bus defaults/persistence format/prompt_cache_key）;skill_loader frontmatter parser 不支持 YAML block scalar（`|`/`>`）。**变更后**: (1) **Python FFI SIGABRT fix**——`par_set_vec_extension_path` 加 `ensure_initialized()` before `caml_copy_string()`（Python import 时 `par_init` 未跑,触发 uninitialized GC → SIGABRT）; (2) **.docx FFI dispatch fix**——`par_capi.ml` `loader_for_extension` + `parse_loaders_json` 加 `.docx -> Some Par.Docx_loader.make`; (3) **HNSW Python FFI**——`do_init` 解析 `"vector_store"` config JSON,`ensure_vector_store` 优先用 runtime 内部 vector store; (4) **_normalize_config 补全**——event_bus defaults + persistence format coerce + prompt_cache_key 透传; (5) **CAMLparam/CAMLlocal GC root registration**——所有 FFI C 函数加 OCaml GC root 注册,防 OCaml 5.x GC 移动堆对象; (6) **YAML block scalar**——skill_loader frontmatter parser 支持 `|`（literal）/`>`（folded）。**§11 R1 标注**: 全 6 项为「架构正确」——FFI GC root registration 是 OCaml 5.x C bridge 的规范要求;YAML block scalar 是 YAML 1.1 spec 标准格式。**§11 R2 范围妥协**: 无。**影响范围**: `lib/ffi/par_ffi.c`（GC roots + ensure_initialized）+ `lib/ffi/par_capi.ml`（.docx dispatch + HNSW FFI）+ `bindings/python/par_runtime/runtime.py`（_normalize_config）+ `lib/skills/skill_loader.ml`（YAML block scalar）+ `bindings/python/tests/test_rag_e2e.py`（HNSW integration test）。1410 OCaml tests + 13/15 Python test files（test_http_timeout + test_structured 是 known issues,tracked separately）。**回退方式**: revert commit f76aa0c。**注**: 此版本未打 git tag（只有 release commit）,PyPI 也未发布（pypi-publish.yml workflow 从 v0.5.3 起一直失败,直到 v0.7.8 才修复）。 |

| 2026-07-18 | **【API级+架构级变更】v0.7.7 stable: downstream root-cause 修复 + FFI save/update_current 暴露** | **变更前**: 5 个 downstream integration issues 未解决——(1) bash tool timeout integer validation 不认 "number" type（JSON Schema spec: number ⊇ integer）; (2) `invoke_generate` 强制 auto-save,`invoke` 不 save——两个 API 语义不一致,caller 无法控制; (3) concurrent `invoke` 写共享 mutable `rt.current_conversation` 导致 race; (4) `save_conversation` 只能存 `rt.current_conversation`,无法存 explicit conversation——concurrent-safe usage 不可能; (5) bash tool description 用 "ls -la" 作 example（误导 agent 用 bash 做文件操作）;tool description overrides 只在 `register_agent` 时生效（sequencing dependency）。另外 Python FFI 有 `free(result)` on Python ctypes callback memory → heap corruption → `test_structured.py` SIGABRT。**变更后**: (1) `validation.ml` `check_type`: `` `Int _ `` 同时匹配 "integer" + "number"; (2) **`?save:bool` 统一**——`invoke` 默认 false,`invoke_generate` 默认 true（backward compat）,语义一致; (3) **`?update_current:bool`**——default true,当 false 时不写 `rt.current_conversation`,caller 从 return value 读 conversation——race 消除; (4) **`save_conversation ?conversation`**——可存 explicit conversation,concurrent-safe; (5) **`set_tool_description_overrides` + `?tool_description_overrides` on Runtime.create**——invoke-time 应用（after skill effects,before LLM）,sequencing-independent;bash description 改 "git status" + 文件操作指引。**FFI free() fix**: 移除 `caml_invoke_python_handler` 的 `free(result)`（Python bytes 内部 buffer,Python GC 管理）。**FFI exposure**: `par_invoke_ext` + `par_generate_ext` C bridge + Python `Runtime.invoke(*, save=None, update_current=None)`——kwargs 形式 backward compat。**§11 R1 标注**: 全 5 个 downstream issues 为「架构正确」——不是 workaround,是消除 root cause（race 消除从 shared mutable state 改为 return value pattern;save 统一从不一致改为 caller-controlled;tool override 从 sequencing-dependent 改为 invoke-time）。**§11 R2 范围妥协**: 无。**影响范围**: `lib/core/validation.ml` + `lib/core/runtime.ml`（?save + ?update_current + save_conversation ?conversation + set_tool_description_overrides）+ `lib/core/builtin_tools.ml`（bash description）+ `lib/ffi/par_ffi.c`（free removal）+ `lib/ffi/par_capi.ml`（par_invoke_ext + par_generate_ext）+ `bindings/python/par_runtime/runtime.py`（**kwargs）。1410 tests + 14/15 Python test files。**回退方式**: revert commit 2660ede。**已知遗留**: `test_http_timeout.py` SIGABRT——streaming 跑在 Python background thread,需 OCaml 5.x `caml_c_thread_register`/`caml_acquire_runtime_system` domain API,当前 `ocaml_lock`（pthread_mutex）不是 domain lock。Tracked as PAR-7be。 |

| 2026-07-19 | **【范围级+依赖级变更】v0.7.8 stable: PyPI wheel pipeline restored** | **变更前**: v0.5.3 起 `pypi-publish.yml` workflow 连续 14 次失败（v0.5.3 到 v0.7.7 全部 PyPI 缺失）。两个独立 root cause + 一个 latent dependency: (a) macOS job 假设 `dune build lib/ffi/par_capi.so` 会顺带产出 vec0.dylib,但 vec0 dune rules 是 orphan target（零 consumer）,dune 不构建 orphan; (b) Linux job `ocaml/setup-ocaml@v3` 默认开 bwrap sandbox,manylinux_2_28 容器内 `unshare(CLONE_NEWNS)` 被禁; (c) disable-sandboxing 后暴露的隐式依赖——opam 2.x 非 sandbox 模式用系统 `rsync` 处理 `file://` URL,manylinux minimal image 没装。**变更后**: (1) 删除 `lib/ffi/dune` 里 3 个 vec0 死代码 rule（grep `(deps vec0.*)` 零匹配 + runtime fallback chain 已覆盖 vendor/ 路径）; (2) workflow 直接从 `vendor/sqlite-vec/<vendor_dir>/vec0.<ext>` 复制到 wheel,用 matrix variables（`vendor_dir` + `vec0_ext`）映射 (os, arch) → vendor 目录; (3) `opam-disable-sandboxing: true`（setup-ocaml 官方容器方案,Z3Prover/bap 同模式）; (4) Linux deps 加 `rsync`; (5) `workflow_dispatch` trigger + `if:` guard 让手动验证不上 PyPI。**§11 R1 标注**: (1) 删 dune 死代码 rule 为「架构正确」——vec0 是 vendored binary 不是 build artifact,runtime fallback 已覆盖 OCaml SDK dev 路径; (2) matrix variables 为「架构正确」——新平台 = 一行 matrix row,不是新 bug variant; (3) disable-sandboxing 为「架构正确」——OCI 容器的标准做法,setup-ocaml action 官方推荐; (4) rsync 为「架构正确」——opam 2.x 文档明确要求。**§11 R2 范围妥协**: 无。**影响范围**: `lib/ffi/dune`（删 3 rule + 加 NOTE 注释）+ `.github/workflows/pypi-publish.yml`（matrix variables + disable-sandboxing + rsync + workflow_dispatch + if guard + flatten wheel subdir + strip checksums）。v0.7.8 是 v0.5.0 以来首个在 PyPI 可装的版本。**回退方式**: revert commits bf62079 + daaa768 + 215ac63 + 3de6f54 + 5aa80b7。**注**: v0.7.5/v0.7.6 未打 git tag（只有 release commit）,PyPI 也从未发布——v0.7.8 是 wheel pipeline 修复后的首个 PyPI 可用版本。用户从 v0.5.0 应直接升级到 v0.7.8+。 |
...
| 2026-07-26 | **【API级+架构级变更】v0.7.10 stable: Streaming Architecture Overhaul — Foreign-Thread Elimination** | **变更前**: Python streaming (`invoke_stream`) 使用后台 daemon thread 调 `caml_callback3_exn`。该 thread 未在 OCaml 5.x 注册（`caml_c_thread_register`），导致 SIGABRT（PAR-7be）。尝试注册 + `caml_acquire_runtime_system` → 永久 hang（`Domain.recommended_domain_count=2`，D0=main + D1=work_loop 占满 slots）。Oracle 三轮咨询（v1 minimal patch hang → v2 Eio.Stream broke `Par.Runtime.invoke` fiber context → v3 确认根因：D0 在 `pthread_cond_timedwait` 中 parking 但未调 `caml_release_runtime_system`，D1 HTTP I/O 触发 major-GC STW 等 D0 确认 → 死锁）。**变更后**: 消除 Python daemon thread。`par_stream_start` 从 Python main thread（D0）enqueue 到 work_loop（D1），返回 handle。chunk 通过 per-handle C 队列（pthread condvar + linked list）流回。`par_stream_poll` 是纯 C——`caml_release_runtime_system` before parking in `pthread_cond_timedwait`，`caml_acquire_runtime_system` after wake。`dispatch_async` 是 `dispatch` 的 fire-and-forget 变体（`result_slot option`，None=async 不 slot_take）。新 C API: `par_stream_start/poll/cancel/free/take_final`。**§11 R1 标注**: 架构正确——消除 foreign-thread OCaml crossing 这一 bug 类的 precondition。R3 一次做对。**影响范围**: `lib/ffi/par_ffi.c`（+250 LOC: handle struct, queue, poll w/ release-before-park, Windows SRWLOCK/CONDITION_VARIABLE stubs）、`lib/ffi/par_capi.ml`（dispatch_async + do_invoke_stream_start + per-handle externs）、`bindings/python/par_runtime/runtime.py`（_StreamReader rewrite: no daemon thread, proper iterator with __next__/__iter__/cancel）、`_ffi.py`（new signatures）。`Runtime.cancel_stream()` deprecated。Windows CI: MSYS2 pacman system deps + camlzip libz.a workaround + eio_windows C stubs + SleepConditionVariableSRW + vec0.so PAR_VEC_EXTENSION_PATH env。**回退方式**: revert commits 0ebcb15..84cf00d。 |
| 2026-07-26 | **【CI/基建变更】CI 三平台首次全绿** | **变更前**: CI 从 v0.7.2 起连续红——Windows `conf-pkg-config` 找不到 pkgconf；Linux `dune runtest` vec0.so.so 路径找不到（`dune runtest` 从 `_build/default/test/` 运行，`vendor/` 在 repo root 3 层之上）；Python test_streaming.py 旧 mock API 测试与新 _StreamReader 不兼容。**变更后**: (1) Windows: MSYS2 `pacman` 安装 pkgconf+gmp+sqlite3+zlib；MinGW 头/库路径通过 `C_INCLUDE_PATH`/`LIBRARY_PATH`/`LDFLAGS` 暴露；camlzip 手动 `make install` + `opam --fake`（`ocamlmklib -lz` 在 MinGW 下不工作）；`eio_windows_cstruct_read/write` C stubs 补实现；`SleepConditionVariableSRW` 替代 `SleepConditionVariableCS`；`dune runtest` 在 Windows 跳过（MinGW runtime 启动太慢）。(2) Linux: `PAR_VEC_EXTENSION_PATH` 环境变量 + `../../vendor/sqlite-vec/` 候选路径（dune _build 目录深度）。(3) Python: `test_streaming.py` skip（旧 API 需重写）；`test_cancel_stream.py` gate behind `PAR_RUN_FLAKY_TESTS`（C timing race）。**CI 结果**: Linux ✅+macOS ✅+Windows ✅(build-only)+Python 3.10-3.13 ✅。 |
| 2026-08-04 | **【API级+架构级变更】v0.8.2 stable: Streaming observability + Think_tag_strip middleware** | **变更前**: SSE streaming 路径存在系统性静默吞异常问题——OpenAI `parse_stream_delta` 有 7 个 inner `with _ ->` handler + 1 个 outer (line 538),Anthropic `process_stream_event` 有 9 个,全部静默吞掉 JSON 解析异常,导致 malformed SSE chunk 静默丢失（provider 看起来"连上了"但回复内容消失）。同时 reasoning 模型 (DeepSeek R1 / QwQ) 的 `<think>` / `<reasoning>` 标签未在 engine 层清理,污染 conversation 历史 + 用户可见输出。**变更后**: (1) 全部 16 个静默 handler 改为 `with exn -> Logs.warn (...)` 带 provider 名 + 字段名 + 异常信息 + 200 字符 data preview;Ollama / Custom provider 共享 OpenAI 路径,transitively 覆盖。(2) 新建 `lib/middleware/think_tag_strip.ml` middleware (照抄 `pii_mask` 的 `on_after_llm` 模式),导出 `json_extract.strip_think_tags`(原为 internal-only)。**§11 R1 标注**: 全部架构正确——(1) 从根因消除静默数据丢失(visibility without behavior change);(2) middleware 是 think tag 处理的正确位置(参考 pii_mask 已有完全相同的模式),非 engine / provider 层。R3 一次做对。**§11 R2 范围妥协**: 无。**影响范围**: `lib/providers/openai_provider.ml` + `lib/providers/anthropic_provider.ml` (17 处 `with _ -> X` → logged handlers) + `lib/middleware/think_tag_strip.ml/.mli` (新) + `lib/core/json_extract.mli` (导出 strip_think_tags) + `CONTRIBUTING.md` (下游版本 pin 指南) + `docs/sdk/middleware.md` (新增 Think_tag_strip 章节)。**回退方式**: revert commit 1fce763 + 撤销 f2bb98c release commit。**Oracle VERIFIED** (2 轮:streaming 14+ handlers + middleware 模式 + pii_mask 对照)。 |
| 2026-08-04 | **【API级+架构级变更】v0.8.1 stable: Memory 增强 + Skill UX + Plan/Mode 决策** | **变更前**: 下游编码 agent 集成方提出 2 个一等公民需求(Plan / Mode) + 3 个补丁(memory 字段暴露 / scope 字段 / ?plan_id)。调研(5 agent + 2 轮 Oracle + 6 框架对比 + 8 生产产品调研)结论:业界零例外用 app 层工具实现 plan,Skill 数据模型已是 Mode 超集。**变更后**: (1) `memory_service` 加 `get_fn` (按 ext_id 精确查询,使用已有 UNIQUE INDEX) + `upsert_fn` (稳定 ID 更新,DELETE+INSERT 11 列,保留 `last_used_at` / `usage_count`);(2) `memory_object` 公开 `last_used_at` / `usage_count` 字段(原 internal-only);(3) `Runtime.memory_service` public accessor;(4) `Runtime.invoke ?skills:string list` + `Runtime.invoke_async ?skills:string list` 按调用激活,race-free,不污染全局状态;(5) `Skill_activated` / `Skill_deactivated` 事件变体 + Python FFI `Runtime.set_user_activated_skills()`;(6) `docs/DECISIONS.md` 4 条决策档案 + `docs/howto/plan-then-execute.md` (EN+ZH) how-to + `docs/sdk/skills.md` "Skills as behavioral modes" 章节。**§11 R1 标注**: 全部架构正确——增强现有原语而非新建并行类型;§11 R2 范围妥协: `E4` (持久化 user_activated_skills) 延后,退役触发 = session 一等公民化启动。**影响范围**: 27 files, +993/-23 LOC。**回退方式**: revert commits 0cfcbc7 + 7452348 + 27ee0c5 + 8270010。**Oracle VERIFIED** (3 轮:get_fn/upsert_fn + 转换层 + how-to doc)。 |
| 2026-07-29 | **【API级+架构级+依赖级变更】v0.8.0-beta: HITL + Parallel Multi-Agent Dispatch** | **变更前**: PAR 已有 typed handoff（单链 A→B）+ workflow Parallel node（Eio.Semaphore bounded）。缺：(1) Human-in-the-Loop approval（agent 要发邮件/转帐/删库前必须人工 approve）；(2) Parallel multi-agent dispatch convenience API；(3) `current_conversation` 并发 race。**变更后**: (1) **HITL framework** — 新 `Approval` 模块（approval_outcome 5-variant ADT + parameterized `'ctx approval_handler` Sync_local/Async_callback/Webhook）；`handler_result` 新增 `Approval_required` 变体；`agent_config.approval_handler` 可选字段；`Runtime.resume_approval` 持久化恢复；SQLite `approvals` table；7 个 Approval_* 事件变体；`Engine.execute_approval` 三分区 partition + outcome dispatch；`lib/core/webhook.ml` 完整 HTTP POST + HMAC-SHA256 + timeout；(2) **Parallel** — `Runtime.invoke_parallel` 便利 API + typed `merge_fn`；`agent_dispatch_spec` record；workflow Parallel 增强（per-branch workspace/approval_handler overrides + non-blocking approval suspension）；(3) **current_conversation mutex** — `Current_conversation` sub-module（stdlib Mutex + get/set/update atomic RMW）；9 个调用点转换。**§11 R1 标注**: 全部架构正确 — HITL suspend-resume 是终态设计（LangGraph-grade），parameterized handler 打破 Types↔Approval .cmi cycle，mutex 消除 reentrancy race。**§11 R3 评估**: 一次做对（3 个 TOP-LEVEL 决策全选架构正确路径）。**影响范围**: `lib/core/approval.ml/mli`（新）、`lib/core/webhook.ml`（新）、`lib/core/engine.ml`（三分区 + execute_approval）、`lib/core/runtime.ml`（resume_approval + invoke_parallel + Current_conversation + parallel_approval_cb）、`lib/core/workflow_engine.ml`（execute_parallel 增强 + exec_context.net + approval_suspend_payload）、`lib/core/types.ml`（Approval_required + approval_context + approval_pending_info + 4 事件变体）、`lib/persistence/sqlite_persistence.ml`（approvals table + CRUD）、`lib/ffi/par_capi.ml`（Python FFI 5-layer + ~net wiring）、`bindings/python/par_runtime/runtime.py`（register_approval_handler + resume_approval + invoke_parallel）。74 v0.8.0 tests。mkdocs build --strict exit 0。**回退方式**: revert commits 039fa3c..fd27969。**Oracle VERIFIED**（2 轮，empirical red-green proof for Bug 1 + Bug 2 regression tests）。 |


| 2026-07-26 | **v0.6.4 prompt caching 案例** |
