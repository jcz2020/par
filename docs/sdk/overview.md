...
The PAR SDK is an OCaml 5.4+ library for building LLM-powered agents. It provides a ReAct reasoning loop, multi-provider LLM abstraction, type-safe shell execution, MCP client integration (stdio + HTTP/SSE), and a 9-middleware pipeline. This overview is the deep dive; the README is the landing.

## What is the PAR SDK?

PAR (Programmable Agent Runtime) ships as one opam package (`par`) plus a PyPI Python binding (`par_runtime`). The `par_cli` CLI package was removed in v0.6.7; the SDK is the production surface — everything is accessible through `Par.Runtime.*` functions. For the interactive coding-agent experience, see [par-code](https://github.com/jcz2020/par-code).
...
- 9 built-in middlewares (logging, retry, rate-limit, timeout, input validation, PII mask, tool-output sanitization, Think_tag_strip)
- Multi-provider: OpenAI compatible + Anthropic Messages API + custom registration
- 1505 OCaml tests + Python bindings passing
...
(The previous CLI shipped as a separate opam package `par_cli` so consumers of `par` did not pull in `cmdliner` and the REPL dependencies; that package was removed in v0.6.7.)
...
The facade `Par` is generated by `lib/par.ml`. Everything links against `par` directly (the `bin/` CLI directory was removed in v0.6.7; the interactive coding-agent product lives in [par-code](https://github.com/jcz2020/par-code)).
...
- [`docs/sdk/middleware.md`](middleware.md): all 9 built-in middlewares and the `middleware_hook` shape
